...
data "template_file" "ansible_hosts" {
  ...
  vars {
    ...
    master_nodes = "${join("\n", formatlist("%s openshift_node_labels=\"{'region': '%s', 'zone': '%s', 'role': 'infra'}\"", openstack_compute_instance_v2.master.*.name, openstack_compute_instance_v2.master.*.region, openstack_compute_instance_v2.master.*.metadata.zone))}"
    compute_nodes = "${join("\n", formatlist("%s openshift_node_labels=\"{'region': '%s', 'zone': '%s', 'role': 'worker'}\"", openstack_compute_instance_v2.node.*.name, openstack_compute_instance_v2.node.*.region, openstack_compute_instance_v2.node.*.metadata.zone))}"
    loadbalancer_node = "${openstack_compute_instance_v2.loadbalancer.0.name}"
...
data "template_file" "dns_entries" {
	template = "${file("files/powerdns/dns_entries.sql")}"
...
resource "null_resource" "deploy_openshift" {
  ...
  triggers {
    cluster_instance_ids = "${join(",", openstack_compute_instance_v2.master.*.id)},${join(",", openstack_compute_instance_v2.node.*.id)},${openstack_compute_instance_v2.infra_host.id}"
  ...
  provisioner "file" {
    content = "${join("\n", formatlist("ssh -o 'StrictHostKeyChecking no' -t %s@%s sudo systemctl status NetworkManager.service", "centos", 
										concat(openstack_compute_instance_v2.master.*.name, 
											   openstack_compute_instance_v2.node.*.name, 
											   openstack_compute_instance_v2.loadbalancer.*.name)))}"
  ...
  provisioner "remote-exec" {
    inline = [
      "cat /tmp/dns_entries.sql | mysql powerdns",
      "rm -f /root/.ssh/known_hosts",
      "chmod +x /tmp/pre-cache-ssh-known-hosts.sh",
      "/tmp/pre-cache-ssh-known-hosts.sh",
